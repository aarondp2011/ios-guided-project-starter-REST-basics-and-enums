...
class PersonController {
    ...
    func searchFoePeopleWith(searchTerm: String, completion: @escaping () -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            ...
            guard let data = data else {
                print("Ni data returned from data task.")
                completion()
                return
            }
            ...
            // Decode and adding object to array
            do {
                let personSearch = try jsonDecoder.decode(PersonSearch.self, from: data)
                self.crew.append(contentsOf: personSearch.results)
            } catch {
                print("Unable to decode data into object of typr PersonSearch: \(error)")
            }
